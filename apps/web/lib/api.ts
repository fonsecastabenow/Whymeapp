...
export type JobData = {
  ...
  education_course_required: string | null
  education_course_is_flexible: boolean
  hard_skills_min_match: number | null
...
export function getCompanyCandidates(companyId: string, authToken: string): Promise<CandidateMatchData[]> {
  return apiFetch(`/matches/company/${encodeURIComponent(companyId)}/candidates`, undefined, authToken)
}
...
// ─── AI Culture Interview ───────────────────────────────────────────────────

export type AICultureStartRequest = {
  company_name: string
  industry?: string | null
  description?: string | null
}

export type AICultureStartResponse = {
  session_id: string
  question: string
  done: boolean
  culture_vector?: Record<string, number> | null
  summary?: string | null
}

export type AICultureAnswerResponse = {
  question: string
  done: boolean
  culture_vector?: Record<string, number> | null
  summary?: string | null
...
export type SuggestOceanResponse = {
  ocean_suggested: Record<string, number>
  justificativa: string
}

export function startAICultureInterview(
  companyId: string,
  data: AICultureStartRequest,
  token: string,
): Promise<AICultureStartResponse> {
  return apiFetch(
    `/companies/${encodeURIComponent(companyId)}/ai-culture/start`,
    { method: "POST", body: JSON.stringify(data) },
    token,
  )
}

export function answerAICultureInterview(
  companyId: string,
  answer: string,
  token: string,
): Promise<AICultureAnswerResponse> {
  return apiFetch(
    `/companies/${encodeURIComponent(companyId)}/ai-culture/answer`,
    { method: "POST", body: JSON.stringify({ answer }) },
    token,
  )
}

export function suggestOceanForJob(
  jobTitle: string,
  jobDescription: string,
  token: string,
): Promise<SuggestOceanResponse> {
  return apiFetch(
    "/jobs/suggest-ocean",
    { method: "POST", body: JSON.stringify({ job_title: jobTitle, job_description: jobDescription }) },
    token,
  )
}
