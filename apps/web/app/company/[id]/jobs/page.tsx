...
function JobFormModal({
  ...
  const [educationCourseRequired, setEducationCourseRequired] = useState(editing?.education_course_required ?? "")
  const [educationCourseIsFlexible, setEducationCourseIsFlexible] = useState(editing?.education_course_is_flexible ?? true)
  const [hardSkillsMinMatch, setHardSkillsMinMatch] = useState<number | "">(editing?.hard_skills_min_match ?? "")
  const [jobPostalCode, setJobPostalCode] = useState(editing?.postal_code ?? "")
  const [acceptanceRadius, setAcceptanceRadius] = useState<number | "">(editing?.acceptance_radius_km ?? "")
  ...
  async function handleAISuggest() {
    ...
    try {
      const res = await suggestOceanForJob(title, description, token)
      setSliders({
        o: Math.round((res.ocean_suggested.o ?? 0.5) * 100),
        c: Math.round((res.ocean_suggested.c ?? 0.5) * 100),
        e: Math.round((res.ocean_suggested.e ?? 0.5) * 100),
        a: Math.round((res.ocean_suggested.a ?? 0.5) * 100),
        n: Math.round((res.ocean_suggested.n ?? 0.5) * 100),
      })
    } catch {
      // Silently fail — user can still use sliders
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const sharedFields = {
        ...
        education_course_required: educationCourseRequired.trim() || null,
        education_course_is_flexible: educationCourseIsFlexible,
        hard_skills_min_match: hardSkillsMinMatch !== "" ? Number(hardSkillsMinMatch) : null,
        postal_code: jobPostalCode.trim() || null,
        acceptance_radius_km: acceptanceRadius !== "" ? Number(acceptanceRadius) : null,
      }
      if (editing) {
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 space-y-5 px-6 py-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-semibold transition-all disabled:opacity-40"
                ...
                >
                  ...
                  {aiSuggestLoading ? "Analisando..." : "Sugerir com IA"}
                </button>
          ...
          {/* ── Filtro de Formação ── */}
          <div className="space-y-3 rounded-[12px] p-4" style={{ background: "rgba(58,176,255,0.04)", border: "1px solid rgba(58,176,255,0.10)" }}>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em]" style={{ color: "rgba(58,176,255,0.7)" }}>
              🎓 Filtro de Formação
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Curso Exigido</label>
                <FieldInput
                  type="text"
                  value={educationCourseRequired}
                  onChange={(v) => setEducationCourseRequired(v as string)}
                  placeholder="Ex: Ciência da Computação"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Flexível?</label>
                <FieldSelect value={educationCourseIsFlexible ? "sim" : "nao"} onChange={(v) => setEducationCourseIsFlexible(v === "sim")}>
                  <option value="sim">Sim (área relacionada)</option>
                  <option value="nao">Não (curso exato)</option>
                </FieldSelect>
              </div>
            </div>
          </div>
          ...
          {/* ── Filtro Geográfico ── */}
          <div className="space-y-3 rounded-[12px] p-4" style={{ background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.10)" }}>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em]" style={{ color: "rgba(16,185,129,0.7)" }}>
              📍 Filtro Geográfico
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">CEP da Vaga</label>
                <FieldInput
                  type="text"
                  value={jobPostalCode}
                  onChange={(v) => setJobPostalCode(v as string)}
                  placeholder="Ex: 35010-000"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Raio (km)</label>
                <FieldInput
                  type="number"
                  min={0}
                  value={acceptanceRadius}
                  onChange={(v) => setAcceptanceRadius(v !== "" ? Number(v) : "")}
                  placeholder="Ex: 50"
                />
              </div>
            </div>
            <p className="text-[10px]" style={{ color: "rgba(200,213,234,0.45)" }}>
              Vagas remotas ignoram este filtro automaticamente.
            </p>
          </div>

          {/* ── Mínimo de Skills ── */}
          <div className="space-y-3 rounded-[12px] p-4" style={{ background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.10)" }}>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em]" style={{ color: "rgba(139,92,246,0.7)" }}>
              🎯 Mínimo de Hard Skills
            </p>
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
                % Mínima de Skills Correspondentes
              </label>
              <FieldInput
                type="number"
                min={0}
                max={100}
                value={hardSkillsMinMatch}
                onChange={(v) => setHardSkillsMinMatch(v !== "" ? Number(v) : "")}
                placeholder="Ex: 50"
              />
              <p className="text-[10px]" style={{ color: "rgba(200,213,234,0.45)" }}>
                Percentual das hard skills exigidas que o candidato precisa ter. Ex: 50% = precisa ter metade das skills.
              </p>
            </div>
          </div>
