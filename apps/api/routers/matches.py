...
from services.matching_service import (
    ...
    check_hard_skills_filter,
    check_location_filter,
    compute_match_score,
)
...
async def trigger_matching(
    ...
    for job in jobs:
        # ── Filtro 1: Formação ─────────────────────────────────────────
        if not check_education_filter(
            candidate_education=candidate.education,
            job_education_level_min=job.education_level_min,
            job_education_course_required=job.education_course_required,
            job_education_course_is_flexible=job.education_course_is_flexible,
        ):
            logger.debug(
                "Candidate %s reprovado no filtro de formação para job %s",
                candidate.id, job.id,
            )
            continue

        # ── Filtro 2: Localização ───────────────────────────────────────
        passed_location = await check_location_filter(
            candidate_postal_code=candidate.postal_code,
            job_postal_code=job.postal_code,
            job_acceptance_radius_km=job.acceptance_radius_km,
            job_work_model=job.work_model,
        )
        if not passed_location:
            logger.debug(
                "Candidate %s reprovado no filtro de localização para job %s",
                candidate.id, job.id,
            )
            continue

        # ── Filtro 3: Hard Skills ───────────────────────────────────────
        if not check_hard_skills_filter(
            candidate_hard_skills=candidate.hard_skills,
            job_hard_skills_required=job.hard_skills_required,
            job_hard_skills_min_match=job.hard_skills_min_match,
        ):
            logger.debug(
                "Candidate %s reprovado no filtro de hard skills para job %s",
                candidate.id, job.id,
            )
            continue
...
@router.get("/company/{company_id}/candidates", response_model=list[CandidateMatchOut])
async def list_company_candidates(
    company_id: str,
    session: AsyncSession = Depends(get_session),
):
    """Return all candidates matched to any job of a company, with full info."""
    try:
        company_uuid = uuid.UUID(company_id)
    except ValueError:
        raise HTTPException(
